...
package com.gacrnd.gcs.sourcecode.am;
...
import static com.gacrnd.gcs.sourcecode.am.ActivityManagerDebugConfig.TAG_WITH_CLASS_NAME;
...
class ActivityStack<T extends StackWindowController> extends ConfigurationContainer
    ...
    void onActivityStateChanged(ActivityRecord record, ActivityState state, String reason) {
        if (record == mResumedActivity && state != ActivityState.RESUMED) {
    ...
    private void completePauseLocked(boolean resumeNext, ActivityRecord resuming) {
        ...
        if (prev != null) {
            ...
            final boolean wasStopping = prev.isState(ActivityState.STOPPING);
    ...
    private void makeInvisible(ActivityRecord r) {
        ...
        try {
            ...
            final boolean deferHidingClient = canEnterPictureInPicture
                    && !r.isState(ActivityState.STOPPING, ActivityState.STOPPED, ActivityState.PAUSED);
            r.setDeferHidingClient(deferHidingClient);
            r.setVisible(false);

            switch (r.getState()) {
                case ActivityState.STOPPING:
    ...
    private boolean resumeTopActivityInnerLocked(ActivityRecord prev, ActivityOptions options) {
        ...
        if (mResumedActivity == next && next.isState(ActivityState.RESUMED)
        ...
        if (pausing && !resumeWhilePausing) {
            ...
            return true;
        } else if (mResumedActivity == next && next.isState(ActivityState.RESUMED)
                && mStackSupervisor.allResumedActivitiesComplete()) {
    ...
    void startActivityLocked(ActivityRecord r, ActivityRecord focusedTopActivity,
        ...
        if (r.getWindowContainerController() == null) {//2
            // 创建appWindowContainerController
            r.createWindowContainer();
        }
        task.setFrontOfTask();
    ...
    final TaskRecord finishTopCrashedActivityLocked(ProcessRecord app, String reason) {
        ...
        if (activityNdx >= 0) {
            ...
            if (r.isState(ActivityState.RESUMED, ActivityState.PAUSING, ActivityState.PAUSED)) {
        ...
        if (hadApp) {
            ...
            if (r.finishing && !skipDestroy) {
                if (DEBUG_STATES) Slog.v(TAG_STATES, "Moving to DESTROYING: " + r
                        + " (destroy requested)");
                r.setState(ActivityState.DESTROYING,
                ...
                r.setState(ActivityState.DESTROYED,
    ...
    final void activityDestroyedLocked(ActivityRecord record, String reason) {
        ...
        if (isInStackLocked(record) != null) {
            if (record.isState(ActivityState.DESTROYING, ActivityState.DESTROYED)) {
